...
function sendMail($mes, $type)
{
	...
	$mail->Host       = "mail.zae-box.by";                                         //"mail.zae-box.by"; // sets GMAIL as the SMTP server
	$mail->Port       = 25;                                                       // set the SMTP port
	$mail->Username   = "";                                    //"no-reply@zae-box.by";  // GMAIL username
	$mail->Password   = "";                                                //"Ohgee4ai"; // GMAIL password
	$mail->From       = "";                                    //$mes["email"];
    $mail->CharSet	  = "utf-8";
	$mail->FromName   = "vstrecha.v-roddome.by | ".$mes["price"];                  //$mes["name"]."| BusinesssCARD.web2web.by | ".$type;
    ...
    $mail->Body = 	'<div style="background: #FFDD00; padding: 20px; font-size: 20px;">								
                            <div>
                                <h1 style="color: #ffffff">Vstrecha<span style="color: rgb(255,184,0)">.V-</span>roddome</h1>
                            </div>
                            ...
                            <div style="color:#007106;font-size:30px;display:inline-block;vertical-align:top;margin:0px;padding:0px">
                                ...
                                <p style="color:#007106;font-size:30px;vertical-align:middle;margin:0px 0px 5px 0px;padding:0px">'.$mes["price"].'</p>
                            </div>

                        </div>';
    $titleReplyTo = "Новая заявка";
    
	$mail->AddReplyTo("MiKrob09@gmail.com","Мэнеджер"); // "breloki@zae-box.by","Webmaster"
	//$mail->AddAddress("jonny8k@mail.ru","О Величайший Мастер");
	$mail->AddAddress("j0nnu@yandex.by","Отдел продаж");
	$mail->IsHTML(true); // send as HTML
	
	if(!$mail->Send()) {
		$tmp =  " - Mailer Error: " . $mail->ErrorInfo;
		echo "Произошла ошибка!"; //.$tmp;
	}
	else{
		echo "Ваша заявка принята. Хорошего дня!"; //Message has been sent
	}
}
?>